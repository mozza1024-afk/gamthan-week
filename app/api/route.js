...
async function getActions() {
  return dbRequest('actions?select=action_code,action_name,action_description&is_active=eq.true&order=sort_order.asc');
}
async function getDiaries(pid) {
  return dbRequest(`diaries?select=id,diary_date,day_number,diary_text,other_action_text,created_at,updated_at&participant_id=eq.${q(pid)}&order=diary_date.asc`);
}
async function actionMapFor(diaries) {
  const ids = (diaries||[]).map(d=>d.id);
  if (!ids.length) return {};
  const [links, actions] = await Promise.all([
    dbRequest(`diary_actions?select=diary_id,action_code&diary_id=in.(${ids.join(',')})`),
    dbRequest('actions?select=action_code,action_name'),
  ]);
  const map = Object.fromEntries((actions||[]).map(a=>[a.action_code,a.action_name]));
  const out={};
  for(const r of links||[]){ if(!out[r.diary_id]) out[r.diary_id]=[]; out[r.diary_id].push({code:r.action_code,name:map[r.action_code]||r.action_code}); }
  return out;
}
async function refreshProgress(pid) {
  try { await dbRequest('rpc/refresh_participant_progress',{method:'POST',body:JSON.stringify({p_participant_id:pid})}); } catch {}
}
...
async function publicData() {
  const [settings, organizations, actions, participants, today] = await Promise.all([
    readSettings(),
    dbRequest('organizations?select=id,organization_code,organization_name&is_active=eq.true&order=sort_order.asc'),
    getActions(),
...
async function register(request) {
  ...
  const birth6=String(b.birth6||'').replace(/\D/g,'');
  ...
  if(!validPhone(phone)) throw new Error('휴대전화번호를 확인해 주세요.');
  if(!validBirth6(birth6)) throw new Error('생년월일 6자리를 확인해 주세요. 예: 650326');
  if(!organizationId) throw new Error('소속기관을 선택해 주세요.');
  if(![7,14,21,28].includes(courseDays)) throw new Error('참여 코스를 선택해 주세요.');
  if(b.privacyConsent!==true) throw new Error('개인정보 수집·이용 동의가 필요합니다.');
...
async function participantLogin(request){
  const b=await bodyJson(request); const phone=normalizePhone(b.phone); const birth6=String(b.birth6||'').replace(/\D/g,'');
  if(!validPhone(phone)||!validBirth6(birth6)) throw new Error('휴대전화번호와 생년월일 6자리를 확인해 주세요.');
  const rows=await dbRequest(`participants?select=*&phone=eq.${q(phone)}&limit=1`); const p=rows?.[0];
  if(!p||p.status==='cancelled') throw new Error('신청정보를 확인할 수 없습니다.');
  if(p.locked_until&&new Date(p.locked_until).getTime()>Date.now()) throw new Error('로그인 시도가 여러 번 실패했습니다. 잠시 후 다시 시도해 주세요.');
  const ok=await verifyHash(birth6,p.birth_hash||p.pin_hash,p.birth_salt||p.pin_salt);
  if(!ok){const n=Number(p.failed_login_count||0)+1;const patch=n>=5?{failed_login_count:0,locked_until:new Date(Date.now()+10*60*1000).toISOString()}:{failed_login_count:n};await dbRequest(`participants?id=eq.${q(p.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(patch)});throw new Error('휴대전화번호 또는 생년월일이 일치하지 않습니다.');}
  await dbRequest(`participants?id=eq.${q(p.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({failed_login_count:0,locked_until:null})});
  return json({success:true,token:await participantSession(p.id)});
}
...
async function saveDiary(request){
  const s=await requireParticipant(request); const b=await bodyJson(request); const [p,today,actions]=await Promise.all([getParticipant(s.sub),effectiveToday(),getActions()]); if(!p) throw new Error('참여자 정보를 찾을 수 없습니다.');
  if(participantStatus(p,today)!=='active') throw new Error('오늘은 감탄일기를 작성할 수 있는 기간이 아닙니다.');
  const text=String(b.diaryText||'').trim(); const codes=Array.isArray(b.actionCodes)?[...new Set(b.actionCodes.map(String))]:[]; const other=String(b.otherActionText||'').trim();
  if(text.length<10||text.length>500) throw new Error('감탄일기는 10자 이상 500자 이하로 작성해 주세요.');
  const allowed=new Set((actions||[]).map(a=>a.action_code)); if(!codes.length||codes.some(c=>!allowed.has(c))) throw new Error('오늘 실천한 행동을 1개 이상 선택해 주세요.');
  const dayNumber=Math.max(1,Math.floor((new Date(`${today}T00:00:00Z`)-new Date(`${p.start_date}T00:00:00Z`))/86400000)+1);
  let existing=await dbRequest(`diaries?select=id&participant_id=eq.${q(p.id)}&diary_date=eq.${q(today)}&limit=1`); let diaryId;
  if(existing?.[0]){diaryId=existing[0].id;await dbRequest(`diaries?id=eq.${q(diaryId)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({diary_text:text,other_action_text:other||null,day_number:dayNumber,updated_at:new Date().toISOString()})});await dbRequest(`diary_actions?diary_id=eq.${q(diaryId)}`,{method:'DELETE'});}else{const rows=await dbRequest('diaries',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({participant_id:p.id,diary_date:today,day_number:dayNumber,diary_text:text,other_action_text:other||null})});diaryId=rows?.[0]?.id;}
  if(codes.length) await dbRequest('diary_actions',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(codes.map(c=>({diary_id:diaryId,action_code:c})))});
...
async function completionPhotos(request){
  ...
  let no=(existing||[]).length+1; for(const file of files){const mime=String(file.type||''); if(!['image/jpeg','image/png','image/webp'].includes(mime)) throw new Error('JPG, PNG, WebP 사진만 등록할 수 있습니다.'); if(file.size>1024*1024) throw new Error('압축 후 사진 1장은 1MB 이하여야 합니다.'); const ext=mime==='image/png'?'png':mime==='image/webp'?'webp':'jpg'; const path=`${p.id}/${Date.now()}-${no}.${ext}`; await uploadPhoto(path,file,mime); await dbRequest('completion_photos',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({participant_id:p.id,photo_no:no,storage_path:path,original_file_name:String(file.name||'photo').replace(/[^a-zA-Z0-9._-]/g,'_').slice(-80),mime_type:mime,file_size_bytes:file.size})}); no++;}
  await refreshProgress(p.id); return json({success:true,message:'완주 인증사진을 등록했습니다.'});
}

async function adminBootstrap(request){
  const b=await bodyJson(request); const setup=String(process.env.ADMIN_SETUP_CODE||'').trim(); if(!setup||setup.length<8) throw new Error('Vercel에 ADMIN_SETUP_CODE를 8자 이상으로 설정해 주세요.');
  const admins=await dbRequest('admin_users?select=id&role=eq.super_admin&is_active=eq.true&limit=1'); if(admins?.length) throw Object.assign(new Error('전체관리자 설정이 이미 완료되었습니다.'),{status:409});
  if(String(b.setupCode||'')!==setup) throw Object.assign(new Error('초기 설정 코드가 일치하지 않습니다.'),{status:401});
  const email=String(b.email||'').trim().toLowerCase(), name=String(b.displayName||'').trim(), password=String(b.password||''); if(!/^\S+@\S+\.\S+$/.test(email)||!name||password.length<10) throw new Error('이메일, 이름, 10자 이상 비밀번호를 확인해 주세요.');
  const h=await hashValue(password); await dbRequest('admin_users',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({email,display_name:name,password_hash:h.hash,password_salt:h.salt,role:'super_admin',organization_id:null,is_active:true})});
  return json({success:true,message:'명륜 전체관리자 계정이 만들어졌습니다.'});
}
async function adminLogin(request){
  const b=await bodyJson(request), email=String(b.email||'').trim().toLowerCase(), password=String(b.password||'');
  const rows=await dbRequest(`admin_users?select=*&email=eq.${q(email)}&is_active=eq.true&limit=1`); const a=rows?.[0]; if(!a||!(await verifyHash(password,a.password_hash,a.password_salt))) throw Object.assign(new Error('관리자 이메일 또는 비밀번호가 일치하지 않습니다.'),{status:401});
  await audit({sub:a.id,role:a.role,org:a.organization_id},'login','admin_user',a.id); return json({success:true,token:await adminSession(a),admin:{displayName:a.display_name,role:a.role,organizationId:a.organization_id}});
}
function adminAllowedSite(admin, site){ return admin.role==='super_admin'||String(site.organization_id||'')===String(admin.org||''); }
async function adminDashboard(request){
  const a=await requireAdmin(request); const [participants,diaries,diaryActions,actions,photos,sites,entries,settings,organizations]=await Promise.all([
    dbRequest('participants?select=id,display_name,phone,organization_id,organization_name_snapshot,course_days,start_date,end_date,status,is_completed,created_at&status=neq.cancelled&order=created_at.desc'),
    dbRequest('diaries?select=id,participant_id,diary_date,day_number,diary_text,other_action_text,created_at,updated_at&order=diary_date.desc'),
    dbRequest('diary_actions?select=diary_id,action_code'),
    dbRequest('actions?select=action_code,action_name'),
    dbRequest('completion_photos?select=id,participant_id,photo_no,storage_path,created_at'),
    dbRequest('offline_sites?select=*&is_active=eq.true&order=sort_order.asc'),
    dbRequest('offline_distribution_entries?select=*&order=entry_date.desc,created_at.desc'),readSettings(),
    dbRequest('organizations?select=id,organization_name&is_active=eq.true&order=sort_order.asc')
  ]);
...
async function adminDistribution(request){
  ...
  const distributed=Math.max(0,Number(b.distributedQty||0)), submitted=Math.max(0,Number(b.submittedQty||0)); if(!Number.isInteger(distributed)||!Number.isInteger(submitted)||(distributed===0&&submitted===0)) throw new Error('오늘 배포 또는 제출 수량을 입력해 주세요.');
...
export async function GET(request){
  try{
    const p=pathParts(request); const key=p.join('/');
    if(key==='public-data') return publicData();
    if(key==='dashboard') return participantDashboard(request);
    if(key==='admin/data') return adminDashboard(request);
    if(key.startsWith('admin/photo/')) return adminPhoto(request,p[2]);
...
export async function POST(request){
  try{
    const p=pathParts(request); const key=p.join('/');
    if(key==='register') return register(request);
    if(key==='login') return participantLogin(request);
    if(key==='diaries') return saveDiary(request);
    if(key==='completion-photos') return completionPhotos(request);
    if(key==='admin/bootstrap') return adminBootstrap(request);
    if(key==='admin/login') return adminLogin(request);
